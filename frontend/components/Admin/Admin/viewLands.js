...
function LandView() {
    var ViewLands = reactLocalStorage.getObject('ViewLands');
    ...
    const [price, setPrice] = useState(ViewLands[4]);
    const [image, setImage] = useState('https://res.cloudinary.com/nadun/image/upload/v1618560570/'+ViewLands[5]);
	const [district, setDistrict] = useState(ViewLands[6]);
	const [address, setAddress] = useState(ViewLands[7]);
    ...
    <div className="container">
     <MDBCard style={{ maxWidth: '100%', marginTop:'10%' ,  marginBottom:'10%' }}>
      <MDBRow className='g-0'>
        ...
        <MDBCol md='8'>
          <MDBCardBody>
            <MDBCardTitle>Rs. {price} Land In {district} </MDBCardTitle>
            <MDBCardText>
             {description}
            </MDBCardText>
            <MDBCardText>
        ...
        <div className="text-end mb-3" style={{paddingRight:'5%'}}>
             <a href="/landBuy?minprice=1&maxprice=1&discrict='null'" > <MDBBtn color="black" size='sm'>View More</MDBBtn></a>
        </div>
      </MDBRow>
    </MDBCard>
    </div>
